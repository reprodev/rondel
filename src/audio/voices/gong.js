...
export function play(ctx, destination, time, params) {
  ...
  noiseFilter.frequency.linearRampToValueAtTime(1500, time + 1.0);
  const noiseGain = ctx.createGain();
  const noiseLevel = 0.05 * velocity;
  noiseGain.gain.setValueAtTime(0, time);
  noiseGain.gain.linearRampToValueAtTime(noiseLevel, time + 0.030);
  noiseGain.gain.exponentialRampToValueAtTime(floor(0), time + 0.030 + 2.500);
  noiseGain.gain.setValueAtTime(0, time + 2.530);
  noiseSrc.connect(noiseFilter).connect(noiseGain).connect(destination);
  noiseSrc.stop(time + 2.6);
  // Sine fundamental
  const osc = ctx.createOscillator();
  osc.type = 'sine';
  osc.frequency.value = 200;
  const oscGain = ctx.createGain();
  const oscLevel = 0.10 * velocity;
  oscGain.gain.setValueAtTime(0, time);
  oscGain.gain.linearRampToValueAtTime(oscLevel, time + 0.030);
  oscGain.gain.exponentialRampToValueAtTime(floor(0), time + 0.030 + 2.500);
  oscGain.gain.setValueAtTime(0, time + 2.530);
